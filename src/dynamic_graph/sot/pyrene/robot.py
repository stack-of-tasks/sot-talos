...
class Robot(Talos):
    ...
    def defineHalfSitting(self, q):
        ...
        # torso
        self.setJointValueInConfig(
            q, ["torso_{}_joint".format(i + 1) for i in range(2)], [0.0, 0.006761]
        )
        # left arm
        self.setJointValueInConfig(
            q,
        ...
        # grippers
        self.setJointValueInConfig(
            q, ["gripper_left_joint", "gripper_right_joint"], [-0.005, -0.005]
        )
        # torso
        self.setJointValueInConfig(
            q, ["head_{}_joint".format(i + 1) for i in range(2)], [0.0, 0.0]
        )

    def __init__(self, name, device=None, tracer=None, fromRosParam=None):
        Talos.__init__(
            self, name, device=device, tracer=tracer, fromRosParam=fromRosParam
        )
        """
